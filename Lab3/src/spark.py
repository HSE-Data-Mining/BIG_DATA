...
mlflow.set_tracking_uri("file:/BD_lab_3/logs/mlruns")
...
with mlflow.start_run(run_name="DataIngestion"):
    initial_data = load_initial_data("/BD_lab_3/data/retail.csv")
    initial_data.write.format("delta").mode("overwrite").save("/BD_lab_3/data/bronze/retail_data")

with mlflow.start_run(run_name="DataProcessing"):
    bronze_data = spark.read.format("delta").load("/BD_lab_3/data/bronze/retail_data")
    ...
    save_data(train_encoded, test_encoded, "/BD_lab_3/data/silver")
    optimize_delta_table("/BD_lab_3/data/silver/train")
    optimize_delta_table("/BD_lab_3/data/silver/test")

with mlflow.start_run(run_name="FeatureEngineering"):
    train_data = spark.read.format("delta").load("/BD_lab_3/data/silver/train")
    test_data = spark.read.format("delta").load("/BD_lab_3/data/silver/test")
    ...
    final_train.write.format("delta").mode("overwrite").save("/BD_lab_3/data/gold/train")
    final_test.write.format("delta").mode("overwrite").save("/BD_lab_3/data/gold/test")

final_train = spark.read.format("delta").load("/BD_lab_3/data/gold/train")
final_test = spark.read.format("delta").load("/BD_lab_3/data/gold/test")
